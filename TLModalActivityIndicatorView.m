...
static CGFloat const kModalSize = 160.0;
static CGFloat const kCornerRadius = 20.0;
static CGFloat const kLabelHeight = 30.0;
static CGFloat const kStartingScale = 1.8;
static NSTimeInterval const kAnimationDuration = 0.25;

@interface UIFont (ActivityIndicatorFonts)

+ (instancetype)labelFont;

@end

@implementation UIFont (ActivityIndicatorFonts)

+ (instancetype)labelFont {
    return [[self class] fontWithName:@"Helvetica" size:18.0];
}

@end

@interface UIColor (ActivityIndicatorColors)

+ (instancetype)modalColor;

@end

@implementation UIColor (ActivityIndicatorColors)

+ (instancetype)modalColor {
    return [[self class] colorWithWhite:0.3 alpha:0.8];
}
...
- (instancetype)initWithText:(NSString *)text {
    ...
    if (self) {
        ...
        backgroundLayer.backgroundColor = [UIColor modalColor].CGColor;
